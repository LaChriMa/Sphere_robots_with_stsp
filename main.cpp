...
* simType:
* 	choose which Robot: Barrel or Sphere, 
* 	and if Sphere: how many of them
* env: 
*	choose the environment, for instance between maze or OP
*
*/
...
public:
  ...
  double friction;
  Pos RobInitPos;
  enum SimType {Barrel, Sphere, TwoSpheres, SixSpheres };
  ...
  **		    RU: 3 little trenchs  */
  ...
  /** Parameters to set to define basic qualities of the simulation */
  bool track = true; 
  bool randObstacles = false;
  bool extraGroundPlane = false;
  bool thirdGroundPlane = false;
  bool GroundBox = false;
  double traceLen = 100;
  Env env = NO;
  SimType simType = Sphere;
  /** End */



  int numAgents = (simType==Barrel or simType==Sphere)?1:(simType==TwoSpheres?2:6);
  ...
  void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global){
    ...
    /** create a plane to test if the robots moves differently than on the default ground */
	if(extraGroundPlane==true){
    	...
    	/** create a second plane to test if the robots moves differently than before */
		if(thirdGroundPlane==true){
    ...
    /** create a box to test the difference in the rolling modes compared to the plane */
	if(GroundBox==true){
  ...
  /*********** ROBOTS  **********/
	/** General settings of the sphere robot */
	SphereRobotConf sconf = SphereRobot::getDefaultConf();
	sconf.diameter = 0.5;
	sconf.motorpowerfactor = 120;

  if(simType == Sphere or simType == TwoSpheres){
		...
		/** plotTypes=8 == Noise, Default == Controller as last argument in wiring constructor */
		...
		if(track == true){
			TrackRobot* TrackOpt = new TrackRobot(true, false, false, true); 
			TrackOpt->conf.displayTraceDur = traceLen; /** length of track line */
			TrackOpt->conf.displayTraceThickness = 0.00; /** thickness of track line, if 0 then it is a line */
			agent->setTrackOptions( *TrackOpt );
		}
	}

  if(simType == TwoSpheres){
		OdeRobot* robot2 = new SphereRobot( odeHandle, osgHandle.changeColor(Color(0.,1.,1.)), sconf, 
		...
		One2OneWiring* wiring2 = new One2OneWiring( new WhiteNormalNoise() ); 
		...
		TrackRobot* TrackOpt2 = new TrackRobot(true, false, false, true); 
		TrackOpt2->conf.displayTraceDur = traceLen; /** length of track line */
  ...
  if(simType == SixSpheres){
		/** colors of all modes as used in the paper */
		Color colors[6];
		colors[0] = Color(0.580392, 0, 0.827451);	// T1 = dark_violet
		colors[1] = Color(0,1,1);					// T2 = cyan
		colors[2] = Color(1, 0.647059, 0);			// C1 = orange
		colors[3] = Color(0,0,1);					// S1 = blue
		colors[4] = Color(1,0,0);					// S2 = red
		colors[5] = Color(0,0.392157,0);			// S3 = dark-green
	
		double dist = 2.2;  /** distance between the initial positions of the six robots */

		for(int i=0; i<6; i++){
			OdeRobot* rob = new SphereRobot( odeHandle, osgHandle.changeColor(colors[i]), sconf, 
		 									 "Sphere"+std::itos(i), global.odeConfig, 0.4);
			rob->addSensor(std::make_shared<SpeedSensor>( 1, SpeedSensor::Translational ),Attachment(-1));
			rob->place(Pos( (i<3?-i*dist:-(i-3)*dist) , (i<3 ? 0:1*dist) ,0));
			STSPController* cont = new STSPController( global.odeConfig, i );
			One2OneWiring* wiri = new One2OneWiring( new WhiteNormalNoise() );
			OdeAgent* age = new OdeAgent( globalData );
			age->init( cont, rob, wiri );
			global.agents.push_back( age );
			global.configs.push_back( cont );
			global.configs.push_back( rob );
			if(i==0){ /** workaround because these pointers are needed in command() */
				robot= rob;
				agent= age;
				controller= cont;
			}
			TrackRobot* TO = new TrackRobot(false, false, false, true); 
			TO->conf.displayTraceDur = 200; /** length of track line */
			TO->conf.displayTraceThickness = 0.0; /** thickness of track line, if 0 then it is a line */
			age->addTracking(0, *TO, colors[i]);
	  }
		/** good camera settings for this simulation */
		setCameraHomePos(Pos(-2, 1, 17),  Pos(-180, -90, 0));
  ...
  if(simType == Barrel){
       ...
       robot = new BarrelRobot(odeHandle, osgHandle.changeColor(Color(0.0,0.0,1.0)), conf, "Barrel", 0.4, 2); 
	...
	if(!pause){
		if(friction>0){
			for(int i=0; i<numAgents; i++){
				OdeRobot* rob = globalData.agents[i]->getRobot();
				Pos vel = rob->getMainPrimitive()->getAngularVel();
				rob->getMainPrimitive()->applyTorque(-vel*friction);
			}
		}
	}
  }
  ...
  void createEnv( const OdeHandle& odeHandle, const OsgHandle& osgHandle, 
    ...
    case OP: { 
		...
		setCameraMode( Follow );
		setCameraHomePos(Pos(0.149249, 0.434834, 21.4279),  Pos(179.487, -89.6461, 0));
		setCameraMode( Static );
  ...
  virtual bool command(const OdeHandle&, const OsgHandle&, GlobalData& globalData, int key, bool down){
    if (down) { // only when key is pressed, not when released
      switch ( (char) key )
      {
		...
		case 'M' : robot->moveToPosition(Pos(-2.2*1,2.2*1,.25)); break;
